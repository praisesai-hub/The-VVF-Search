...
val googleServicesConfigProvidesWebClientId = run {
  val config = listOf(
    "google-services.json",
    "src/debug/google-services.json",
    "src/release/google-services.json"
  ).map { file(it) }.firstOrNull { it.isFile && it.length() > 0 }
  ...
  if (config == null) {
    ...
    runCatching {
      ...
      clients.orEmpty().any { client ->
        val clientMap = client as? Map<*, *>
        val oauthClients = clientMap?.get("oauth_client") as? List<*>
        oauthClients.orEmpty().any { oauthClient ->
          val oauthMap = oauthClient as? Map<*, *>
          oauthMap?.get("client_type")?.toString() == "3" &&
            !oauthMap?.get("client_id")?.toString().isNullOrBlank()
        }
      }
    }.getOrDefault(false)
  }
...
android {
  ...
  defaultConfig {
    ...
    // Validation builds may omit Firebase configuration or receive a configuration that
    // does not contain the OAuth web client generated by the Google Services plugin. Supply
    // an empty compile-time fallback in either case; the runtime auth boundary fails closed.
    if (!googleServicesConfigProvidesWebClientId) {
