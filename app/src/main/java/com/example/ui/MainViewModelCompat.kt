...
private fun MainViewModel.compatState(): VmCompatState =
    synchronized(compatStates) { compatStates.getOrPut(this) { VmCompatState() } }
...
val MainViewModel.filteredFiles: StateFlow<List<FileItemEntity>>
    get() = files
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.categoryStats: StateFlow<List<CategoryStat>>
    get() = dashboardStats
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
...
val MainViewModel.level3VisualDuplicates: StateFlow<List<DuplicateGroup>>
    get() = repository.getVisualDuplicates(similarityThreshold).stateInCompat(this, emptyList())
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.videoDuplicates: StateFlow<List<DuplicateGroup>>
    get() = repository.getVideoDuplicates(similarityThreshold).stateInCompat(this, emptyList())
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.semanticDuplicates: StateFlow<List<DuplicateGroup>>
    get() = repository.getSemanticDuplicates(similarityThreshold).stateInCompat(this, emptyList())
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
...
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.selectedDuplicateIds: StateFlow<Set<Long>>
    get() = compatState().selectedIds
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.isDuplicateScanning: StateFlow<Boolean>
    get() = repository.isScanning
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.duplicateScanProgress: StateFlow<Float>
    get() = repository.scanProgress
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.isPageLoading: StateFlow<Boolean>
    get() = compatState().pageLoading
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.isVaultUnlocked: StateFlow<Boolean>
    get() = compatState().vaultUnlocked
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
...
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.enteredPin: StateFlow<String>
    get() = compatState().enteredPin
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.pinError: StateFlow<String?>
    get() = compatState().pinError
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.isVaultPinSetupRequired: Boolean
    get() = !repository.hasVaultPin()
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
val MainViewModel.hasBiometricEnrollment: Boolean
    get() = repository.hasBiometricEnrollment()
...
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
fun MainViewModel.startDuplicateScan() {
...
fun MainViewModel.setVaultAutoLockTimeout(timeoutMs: Long) {
    ...
    val normalized =
        timeoutMs.coerceIn(MIN_VAULT_AUTO_LOCK_TIMEOUT_MS, MAX_VAULT_AUTO_LOCK_TIMEOUT_MS)
...
private fun MainViewModel.loadVaultAutoLockSettings(state: VmCompatState) {
    ...
    state.vaultAutoLockTimeoutMs.value =
        persisted.coerceIn(MIN_VAULT_AUTO_LOCK_TIMEOUT_MS, MAX_VAULT_AUTO_LOCK_TIMEOUT_MS)
...
fun MainViewModel.toggleDuplicateSelection(id: Long) {
    ...
    state.selectedIds.value =
        state.selectedIds.value.toMutableSet().apply { if (!add(id)) remove(id) }
}
...
fun MainViewModel.autoSelectExtraDuplicates() {
    ...
    compatState().selectedIds.value =
        level1ExactDuplicates.value.flatMap { it.files.drop(1).map { file -> file.id } }.toSet()
}
...
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
fun MainViewModel.moveToRecycleBin(file: FileItemEntity) {
...
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
fun MainViewModel.emptyRecycleBin() {
    viewModelScope.launch { repository.emptyRecycleBin() }
}

@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
fun MainViewModel.encryptToVault(file: FileItemEntity) {
    viewModelScope.launch { repository.encryptToVault(file) }
}

@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
fun MainViewModel.rescanPhysicalStorage() {
    viewModelScope.launch { repository.rescanPhysicalStorage() }
}

@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
fun MainViewModel.loadNextPage() = Unit
...
private fun MainViewModel.handlePinUnlock(state: VmCompatState, pin: String, now: Long) {
    val unlocked =
        try {
            ...
            state.pinLockedUntilElapsedMs =
                now + (locked.lockedUntilMs - System.currentTimeMillis()).coerceAtLeast(0L)
            state.enteredPin.value = ""
    ...
    state.enteredPin.value = ""
    state.pinError.value =
        if (state.failedPinAttempts >= MAX_VAULT_FAILED_ATTEMPTS) {
...
@Deprecated("Compatibility presentation surface; migrate UI behavior to explicit domain use cases")
fun MainViewModel.onBiometricEnrollmentSuccess(
    result: BiometricPrompt.AuthenticationResult
): Boolean {
    return try {
        val success = repository.completeBiometricEnrollment(result)
        if (!success)
            compatState().pinError.value =
                compatMessage(R.string.pin_error_biometric_enrollment_saved)
        success
    } catch (_: java.security.GeneralSecurityException) {
