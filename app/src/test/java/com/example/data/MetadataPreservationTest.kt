    ...
    fun `test upsert preserves existing enriched metadata and state`() {
        ...
        val merged =
            existing.copy(
                ...
                md5Hash =
                    if (freshScanned.md5Hash.isNotBlank()) freshScanned.md5Hash
                    else existing.md5Hash,
                ocrText =
                    if (freshScanned.ocrText.isNotBlank()) freshScanned.ocrText
                    else existing.ocrText,
                tags = if (freshScanned.tags.isNotBlank()) freshScanned.tags else existing.tags,
                originalPath =
                    if (freshScanned.originalPath.isNotBlank()) freshScanned.originalPath
                    else existing.originalPath,
                visualSimilarityHash =
                    if (freshScanned.visualSimilarityHash.isNotBlank())
                        freshScanned.visualSimilarityHash
                    else existing.visualSimilarityHash,
                documentCandidateFingerprint =
                    if (freshScanned.documentCandidateFingerprint.isNotBlank())
                        freshScanned.documentCandidateFingerprint
                    else existing.documentCandidateFingerprint,
                semanticEmbeddingVersion =
                    if (freshScanned.semanticEmbeddingVersion > 0)
                        freshScanned.semanticEmbeddingVersion
                ...
                semanticEmbeddingString =
                    if (freshScanned.semanticEmbeddingString.isNotBlank())
                        freshScanned.semanticEmbeddingString
