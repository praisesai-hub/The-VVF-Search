...
class AppDatabaseMigrationTest {
    ...
    fun testMigrationFrom2To3PreservesDataAndAddsColumns() {
        ...
        assertEquals(
            "/storage/emulated/0/DCIM/existing_photo.jpg",
        ...
        // 4. Verify that new columns were successfully added with default values (Correctness
        // requirement B)
        ...
        val pluginsCursor =
            db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='plugins'")
        assertTrue(
            "Plugins table should be created as part of v3 migration",
            pluginsCursor.moveToFirst(),
        )
        pluginsCursor.close()
    ...
    fun testMissingMigrationWithoutFallbackThrowsException() {
        ...
        // and with fallbackToDestructiveMigration disabled. This should throw
        // IllegalStateException.
        try {
            val roomDb =
                Room.databaseBuilder(context, AppDatabase::class.java, "test_migration_db")
                    // Explicitly do not add migration MIGRATION_2_3, and do not call
            ...
            kotlinx.coroutines.runBlocking { roomDb.fileDao().getFileById(101) }
            fail(
                "Expected IllegalStateException due to missing migration path without " +
                    "fallback fallbackToDestructiveMigration"
            )
        } catch (e: IllegalStateException) {
            // Success: expected exception thrown
            assertTrue(
                e.message?.contains("migration") == true || e.message?.contains("Migration") == true
            )
        }
    }
    ...
    fun testMigrationFrom1To2CreatesNewTables() {
        ...
        val vaultCursor =
            db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='vault_items'")
        assertTrue(vaultCursor.moveToFirst())
        vaultCursor.close()

        val cloudCursor =
            db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='cloud_sync'")
    ...
    fun testMigrationFrom5To6AddsCloudOperationState() {
        ...
        val indexCursor =
            db.query(
                "SELECT name FROM sqlite_master WHERE type='index' AND name='index_cloud_sync_operationId'"
            )
        assertTrue(indexCursor.moveToFirst())
        indexCursor.close()
        val workTableCursor =
            db.query("SELECT name FROM sqlite_master WHERE type='table' AND name='work_operations'")
    ...
    fun testMigrationFrom6To7AddsVideoEvidenceColumns() {
        ...
        val db = helper.writableDatabase
        db.execSQL(
            "CREATE TABLE files (id INTEGER PRIMARY KEY NOT NULL, name TEXT NOT NULL, " +
                "path TEXT NOT NULL, category TEXT NOT NULL, sizeBytes INTEGER NOT NULL)"
        )
        db.execSQL(
            "INSERT INTO files (id, name, path, category, sizeBytes) " +
    ...
    fun testMigrationFrom7To8AddsDocumentCandidateFingerprintColumn() {
        ...
        assertEquals(
            "legacy-candidate",
            cursor.getString(cursor.getColumnIndexOrThrow("documentCandidateFingerprint")),
        )
        cursor.close()
        db.close()
    }
    ...
    fun testMigrationFrom8To9AddsCloudTransferStateColumns() {
        ...
        val db = helper.writableDatabase
        db.execSQL(
            "CREATE TABLE cloud_sync (id INTEGER PRIMARY KEY NOT NULL, operationId TEXT NOT NULL, status TEXT NOT NULL)"
        )
        db.execSQL(
            "INSERT INTO cloud_sync (id, operationId, status) VALUES (901, 'op-901', 'QUEUED')"
        )
        ...
        val cursor =
            db.query(
                "SELECT remoteFileId, resumableSessionUri, resumableBytesCommitted FROM cloud_sync WHERE id = 901"
            )
        assertTrue(cursor.moveToFirst())
    ...
    fun testMigrationFrom9To10CreatesFileOperationTable() {
        ...
        db.execSQL(
            "INSERT INTO file_operations (operationId, operationType, fileId, sourcePath, " +
                "targetPath, status, createdAtMs, updatedAtMs, lastErrorCode) " +
                "VALUES ('file-DELETE-1', 'DELETE', 1, '/source', '', 'PREPARED', 10, 10, NULL)"
        )

        val cursor =
            db.query(
                "SELECT operationType, status FROM file_operations WHERE operationId = 'file-DELETE-1'"
            )
        assertTrue(cursor.moveToFirst())
    ...
    fun testFullMigrationPathFrom1To4PreservesData() {
        ...
        assertEquals(
            "",
            filesCursor.getString(filesCursor.getColumnIndexOrThrow("visualSimilarityHash")),
        )
        filesCursor.close()
