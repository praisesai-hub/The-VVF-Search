...
class DuplicateManagerTest {
    ...
    class FakeFileDao : FileDao {
        ...
        override suspend fun moveFilesToRecycleBinAtomic(files: List<FileItemEntity>) {
            ...
            files.forEach { filesMap[it.id] = it }
        }
        ...
        override fun getDuplicateFilesByHash(): Flow<List<FileItemEntity>> =
            flowOf(duplicateFiles.toList())
        ...
        override suspend fun getFileByPath(path: String): FileItemEntity? =
            filesMap.values.find { it.path == path }
        ...
        override suspend fun getAllOrdinaryFilesDirect(): List<FileItemEntity> =
            filesMap.values.filter { !it.isVault && !it.isRecycleBin }
    ...
    @Test
    fun same_file_id_in_multiple_duplicate_groups_results_in_single_recycle_bin_operation() =
        runBlocking {
    ...
    fun cleanSelectedDuplicates_preservesOriginalPathAndUsesSingleAtomicBatch() = runBlocking {
        ...
        val exactMatch =
            withOriginalPath.copy(
                id = 13L,
                name = "second-copy.jpg",
                path = "/files/second-copy.jpg",
            )
        ...
        assertEquals(
            "Atomic move calls: ${fakeDao.moveFilesToRecycleBinAtomicCallCount}",
            1,
            fakeDao.moveFilesToRecycleBinAtomicCallCount,
        )
