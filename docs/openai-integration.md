...
Use the OpenAI Responses API for assistant-style reasoning and structured answers. OpenAI's current documentation identifies Responses as the primary API for text generation and supports streaming and tool use. Keep model selection server-side so the Android app cannot arbitrarily increase cost.
