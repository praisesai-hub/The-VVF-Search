...
Android CI run `32389660487` verified that application-scoped JaCoCo instrumentation raises JVM instruction coverage from the earlier 6.00% baseline to 23.06% (`19,321/83,797`). The required JVM aggregate 70.00% and all scoped JVM gates still fail. The same run's instrumented aggregate passed at 74.20%, but the security, storage, vault, and cloud-sync scoped gates remain below their configured floors. These are release blockers and are not grounds to weaken the production coverage policy.
